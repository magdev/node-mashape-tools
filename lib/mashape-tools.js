...
var extend = require('xtend'),
    ...
    clientIP = function(req, useProxy, debug) {
		if (req.ips.length) {
			...
			if (debug) {
				console.log('Multiple Proxy IPs: %s', req.ips.join(', '));
			}
			if (useProxy) {
				ip = req.ips.slice(1).shift();
				if (debug) {
					console.log('Using IP #2: %s', ip);
				}
				return ip;
			}
    ...
    ipFilter: function(config) {
        ...
        return function(req, res, next) {
            try {
                ...
                if (config.whitelist && config.whitelist.indexOf(clientIP(req, false, config.debug)) !== -1) {
