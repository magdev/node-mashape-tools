...
var extend = require('xtend'),
     ...
     * @param {Function} next
     */
    checkWhitelist = function(config, user, ip, next) {
     ...
     * @param {Function} next
     */
    handleError = function(config, e, next) {
        if (config.strict) {
            return next(e);
        }
        if (config.debug) {
...
module.exports = exports = {
        ...
        return function(req, res, next) {
            try {
                ...
                if ((!iplist || !iplist.length) && (!config.whitelist || !config.whitelist.length)) {
                    handleError(config, new Error('No IP-list or whitelist defined, skipping ipFilter()'), next);
                }
                
                // Check if IP is whitelisted
                checkWhitelist(config, user, ip, next);
        ...
        return function(req, res, next) {
            try {
                ...
                if (!config.proxySecret && (!config.additionalHeaderChecks || !config.additionalHeaderChecks.length)) {
                    handleError(config, new Error('No Proxy-Secret or additional header checks defined, skipping headerFilter()'), next);
                }
     ...
     * @returns {Function}
     */
    serviceContainer: function(config) {
        var ServiceContainer = require('./service-container'),
            defaultCfg = extend(defaultConfig, {
                mashapeKey: (process.env.MASHAPE_KEY || null),
                autodiscovery: true
            });
        
        config = extend(defaultCfg, (config || {}));
        
        return function(req, res, next) {
            req.service = new ServiceContainer(config);
            return next();
        };
    }
};